...
class StockFlowApp {
  ...
  getPageContent() {
    ...
    try {
      switch (this.currentPage) {
        ...
        case 'crm-approvals':
          return window.crmApprovalModule ? crmApprovalModule.render(this) : '<div class="card"><h2>CRM Approvals module not loaded</h2></div>';
        case 'crm-leads':
          return window.crmLeadsModule ? crmLeadsModule.render(this) : '<div class="card"><h2>CRM Leads module not loaded</h2></div>';
        case 'messaging':
  ...
  async renderTopbar() {
    ...
    // Load custom header titles from database
    let pageTitles = {
      'dashboard': '📊 Dashboard',
    ...
    try {
      ...
      if (data.success && data.titles) {
        pageTitles = {
          'dashboard': data.titles.dashboard_title || '📊 Dashboard',
          'products': data.titles.products_title || '📦 Item Management',
          'customers': data.titles.customers_title || '👥 Customers',
          'sales': data.titles.sales_title || '💰 Sales',
          'messaging': data.titles.messaging_title || '💬 Messaging',
          'users': data.titles.users_title || '👤 User Management',
          'settings': data.titles.settings_title || '⚙️ Settings'
        };
      }
    } catch (error) {
  ...
  // Add this new helper method after renderTopbar
  async updateTopbarTitle() {
    try {
      ...
      if (data.success && data.titles) {
        ...
        if (topbarTitleEl && this.currentPage) {
          topbarTitleEl.textContent = pageTitles[this.currentPage] || 'StockFlow';
        }
      }
    } catch (error) {
          ...
          <form id="createRoleForm">
            ...
            <div class="form-group">
              ...
              <div style="background: #f9fafb; padding: 16px; border-radius: 8px; border: 1px solid #e5e7eb;">
                ...
                <!-- CRM -->
<label style="display: flex; align-items: center; padding: 10px; margin-bottom: 8px; border-radius: 6px; cursor: pointer; transition: all 0.2s;" onmouseover="this.style.background='#f3f4f6'" onmouseout="this.style.background='transparent'">
  <input type="checkbox" name="role_permissions" value="crm" style="margin-right: 12px; width: 18px; height: 18px; cursor: pointer;">
  <span style="font-size: 20px; margin-right: 8px;">🎯</span>
  <div>
    <p style="margin: 0; font-weight: 600; font-size: 14px; color: #1f2937;">CRM</p>
    <p style="margin: 0; font-size: 12px; color: #6b7280;">Manage customer relationships and leads</p>
  </div>
</label>
...
<!-- New Chat Modal -->
  ...
  openEditRoleModal(roleId) {
    ...
    const allPermissions = [
      ...
      { id: 'crm', icon: '🎯', name: 'CRM', desc: 'Manage customer relationships and leads' },
    ...
    setTimeout(() => {
      switch (this.currentPage) {
        ...
        case 'sales':
          ...
          break;
        case 'crm':
          if (window.crmDashboardModule) {
            crmDashboardModule.attachListeners(this);
            console.log('✅ CRM Dashboard listeners attached');
          }
          break;
        case 'crm-approvals':
          if (window.crmApprovalModule) {
            crmApprovalModule.attachListeners(this);
            console.log('✅ CRM Approvals listeners attached');
          }
          break;
        case 'crm-leads':
          if (window.crmLeadsModule) {
            crmLeadsModule.attachListeners(this);
            console.log('✅ CRM Leads listeners attached');
          }
          break;
        case 'messaging':
          if (window.messagingPageModule) {
