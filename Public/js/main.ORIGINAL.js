...
class StockFlowApp {
  ...
  async render() {
    ...
    if (this.currentPage === 'login') {
      ...
      const topbar = await this.renderTopbar();
  ...
  async renderSidebar() {
    ...
    if (menuItems.length === 0) {
      console.warn('⚠️ Using fallback hardcoded menu items');
        ...
        <div class="modal-content" style="max-width: 600px;">
          ...
          <form id="createRoleForm">
            ...
            <div class="form-group">
              ...
              <div id="createRolePermissionsContainer" style="background: #f9fafb; padding: 12px; border-radius: 8px; border: 1px solid #e5e7eb; max-height: 280px; overflow-y: auto;">
                <!-- Permissions will be loaded dynamically from active menu items in database -->
                <p style="text-align: center; color: #6b7280; padding: 20px;">Loading active menu items...</p>
              </div>
            </div>
      ...
      <div id="editRoleModal" class="modal">
        <div class="modal-content" style="max-width: 600px;">
          ...
          <form id="editRoleForm">
            ...
            <div class="form-group">
              <label class="form-label">Menu Permissions * (Select at least one)</label>
              <div id="editRolePermissionsContainer" style="background: #f9fafb; padding: 12px; border-radius: 8px; border: 1px solid #e5e7eb; max-height: 280px; overflow-y: auto;">
...
<!-- New Chat Modal -->
  ...
  openAddUserModal() {
    ...
    document.getElementById('userPassword').required = true;
    document.getElementById('userSubmitBtn').textContent = 'Create User';
    document.getElementById('userCanBroadcast').checked = false;
  ...
  openEditUser(id) {
    ...
    document.getElementById('userCanBroadcast').checked = user.can_broadcast === 1;
  ...
  async handleUserFormSubmit(e) {
    ...
    const canBroadcast = document.getElementById('userCanBroadcast').checked ? 1 : 0;
        ...
        <div class="modal-content">
          ...
          <form id="userForm">
            ...
            <div class="form-button-group">
              ...
              <button type="button" class="btn-secondary" onclick="window.app.closeUserModal()">Cancel</button>
            </div>
          </form>
        </div>
      </div>
  ...
  async openCreateRoleModal() {
    ...
    // Load active menu items dynamically from database
    if (window.roleMenuPermissionsModule) {
      await roleMenuPermissionsModule.loadCreateRolePermissions();
    }
  }
  ...
  async openEditRoleModal(roleId) {
    ...
    // Load active menu items dynamically from database with role's current permissions
    const rolePermissions = Array.isArray(role.permissions) ? role.permissions : [];
    if (window.roleMenuPermissionsModule) {
      await roleMenuPermissionsModule.loadEditRolePermissions(rolePermissions);
    }
  ...
  async handleCreateRoleSubmit(e) {
    ...
    try {
      ...
      // ⭐ Re-render to update menu and navigation
      console.log('🔄 Re-rendering sidebar with new role permissions...');
      await this.render();
      this.attachGlobalListeners();

      // ⭐ Reload roles in users page dropdown
      if (this.currentPage === 'users' && window.usersPageModule) {
        await window.usersPageModule.loadRoles(this);
      }
      ...
      this.showConfirm(
        '✅ Success',
        `Role "${displayName}" created successfully!\n\n✓ Menu permissions are now available.`,
        null,
        null,
        true  // info only mode
      );
  ...
  async saveRoleUpdate(roleId, displayName, description, permissions, roleName) {
    ...
    try {
      ...
      // ⭐ ALWAYS re-render sidebar to update menu permissions
      console.log('🔄 Re-rendering sidebar and navigation...');
      await this.render();
      this.attachGlobalListeners();
      ...
      this.showConfirm(
        '✅ Success',
        `Role "${displayName}" updated successfully!${isCurrentUserRole ? '\n\n✓ Your menu and permissions have been updated.' : '\n\n✓ Menu refreshed with new permissions.'}`
      );
